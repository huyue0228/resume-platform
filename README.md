...
提交任务 → 查重与有效志愿排序 → 学校/学历准入 → 固定当前投递标准 → PDF 全文提取 → Kernel 契合度评估与能力标签 → 复核或通过入池 → 按标签和 HC 分配部门需求。
...
标签分为专业方向、技术技能、实践经历，按定义提取原文证据。低于 0.8 的置信度转为待核实，不满足必需标签。待复核不产生部门分配或 HC 占用；`AGENT_KERNEL_ROLLOUT=review_only` 保持全部先复核，`enforced` 使用既有契合度阈值。通过后在「职位候选人池」保留入池资格，没有满足标签的部门需求或没有名额时均保持待分配。
...
标准、标签定义、正文或候选人学历/专业变化会使旧资格要求重新评估；切换有效志愿关闭旧入池资格并保留历史。新增 API 位于 `/api/position-pools/config/` 和 `/api/position-pools/members/`；配置使用版本检查，复核、标签修订、分配操作使用记录 revision 检查。入池列表仅向具备全局简历查看权限的用户开放，部门用户继续通过已分配记录访问候选人。
...
- 固定协议副本：`internal/contract/bundle/`，由 resume-contracts 3.0.0 生成。
...
本版本使用 `resume-analysis/v3`，结果为 `resume-application-assessment/v1`，固定契约版本 3.0.0。平台和 Kernel 必须配套升级；HTTP 路径仍为 `/v2/tasks/execute`。v3 每次只接受一个当前投递标准，不兼容旧的多岗位分析请求；全文格式仍为 `ResumeTextV2`。升级前结束或取消所有旧任务，否则 Go 迁移会明确拒绝启动。
