...
简历库通过“入池状态”筛选当前投递的已入池、待分配、已分配或待重新评估候选人。候选人详情中的“入池与分配详情”保留标签修订、原文证据及流转记录；历史入池记录可从 AI 筛选决策查看。任务创建、定时计划和分配编排统一在“任务中心”。
...
「任务中心」统一提供「执行记录 / 定时计划 / 分配任务」。执行记录支持搜索、状态、来源、创建日期、创建人和计划历史筛选，数据库分页查询；概览统计整个查询范围。计划可暂停、恢复、取消或查看全部执行历史；详情和查询条件可通过 URL 直接访问。操作规则、产品参考及 API 见 [任务中心说明](ops/task-center.md)。

部门人员授权通过手工新增或导入维护；首次初始化不再自动创建示例部门人员。

## 技术栈与目录

- Go 1.25：`cmd/resume-platform/`、`internal/platform/`。
- PDF 提取：`internal/pdftext/`；运行环境需要 Poppler 及中文 CMap 数据。
- 固定协议副本：`internal/contract/bundle/`，由 resume-contracts 5.0.0 生成。
...
本版本使用 `resume-analysis/v5`，结果为 `resume-application-assessment/v1`，固定契约版本 5.0.0。HTTP 路径仍为 `/v2/tasks/execute`，每次只接受一个当前投递标准；全文格式仍为 `ResumeTextV2`。不接入 `scope.taxonomy` 和专业词表查询工具。分配使用 `resume-allocation/v1`。

v5 需配套 Contracts 5.0.0 和支持 `resume-analysis/v5` 的 Kernel；旧版冻结任务需重新提交。系统设置中的“AI 单次上下文上限”控制每次输入和输出的总容量，默认 32768；每任务累计额度保持 120000。分析详情的“运行记录与版本”展示具体停止原因、剩余额度、逐轮模型耗时与 Token、历史整理和重复调用；工具的 0ms 表示低于 1ms，不包含模型请求耗时。缓存复用不会沿用原任务的预算消耗记录。
