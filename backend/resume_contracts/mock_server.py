...
from .models import AnalysisRequestV1, PROTOCOL, TOOLSET, RESULT
...
def main():
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            self.reply(200 if self.path=="/healthz" else 404,dict(ok=True,mock=True,build=args.build,
                task_protocol_version=PROTOCOL,task_toolset_version=TOOLSET,task_result_version=RESULT))
        def do_POST(self):
