...
const POOL_STATUS = { pending_review: '入池待复核', pending_allocation: '入池待分配', allocated: '已分配', needs_reanalysis: '需要重新评估', closed: '资格已关闭', rejected: '复核未通过' }
const EVENTS = { assessment_saved: '投递评估完成', review_approved: '复核通过', review_rejected: '复核未通过', tags_revised: '人工修订标签', allocation_waiting: '等待分配', allocated: '完成分配', closed: '关闭入池资格' }
...
export default function PositionPoolsPage() {
  ...
  return <PageContainer title="职位候选人池">
    <Alert showIcon type="info" message="按当前投递评估，通过后进入对应内部职位池" description="待复核不占名额；入池后按标签和部门名额分配。标签不匹配或名额不足时保留资格，调整部门需求后可直接重新分配。列表按记录时间排序。" style={{ marginBottom: 16 }} />
    ...
    <Drawer title={member ? `${member.assessment?.pool?.name} · ${POOL_STATUS[member.status]}` : ''} open={!!member} onClose={close} width={800}>
      {member && <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <Space wrap>
          {canManage && member.status === 'pending_review' && <><Button type="primary" onClick={() => setEdit({ action: 'approve', note: '' })}>复核通过并分配</Button><Button danger onClick={() => setEdit({ action: 'reject', note: '' })}>复核不通过</Button></>}
          {canManage && member.status === 'pending_allocation' && <Button type="primary" loading={busy} onClick={() => act('allocate')}>重新执行分配</Button>}
          {canManage && ['pending_review', 'pending_allocation'].includes(member.status) && <Button onClick={() => setEdit({ action: 'tags', note: '', supported: (member.tags || []).filter((t) => t.status === 'supported').map((t) => t.code), uncertain: (member.tags || []).filter((t) => t.status === 'needs_verification').map((t) => t.code) })}>修订能力标签</Button>}
        ...
        <div><h3>复核与分配记录</h3><Timeline items={(member.events || []).map((e) => ({ key: e.id, children: <><strong>{EVENTS[e.kind] || e.kind}</strong><p>{e.payload?.message || e.payload?.reason || e.payload?.note}</p>{e.kind === 'tags_revised' && <p>修订后：{(e.payload.tags || []).map((t) => `${member.assessment?.tag_catalog?.find((d) => d.code === t.code)?.name || t.code}（${t.status === 'supported' ? '已确认' : '待核实'}）`).join('、') || '无标签'}</p>}<small>{new Date(e.created_at).toLocaleString('zh-CN')}{e.actor_id ? ` · 操作人 ID ${e.actor_id}` : ' · 系统'}</small></> }))} /></div>
      </Space>}
    </Drawer>
    <Modal title={edit?.action === 'tags' ? '修订能力标签' : '复核投递评估'} open={!!edit} onCancel={() => setEdit(null)} confirmLoading={busy} okButtonProps={{ disabled: !edit?.note?.trim() }} onOk={() => edit.action === 'tags' ? act('tags', { note: edit.note, tags: [...edit.supported.map((code) => ({ code, status: 'supported' })), ...edit.uncertain.map((code) => ({ code, status: 'needs_verification' }))] }) : act('review', { decision: edit.action, note: edit.note })}>
      {edit?.action === 'tags' && <>{[['supported', '已确认'], ['uncertain', '待核实']].map(([field, label]) => <div key={field}><p>{label}</p><Select aria-label={label} mode="multiple" style={{ width: '100%' }} value={edit[field]} options={(member?.assessment?.tag_catalog || []).filter((t) => !edit[field === 'supported' ? 'uncertain' : 'supported'].includes(t.code)).map((t) => ({ value: t.code, label: t.name }))} onChange={(value) => setEdit({ ...edit, [field]: value })} /></div>)}</>}
      <p>{edit?.action === 'reject' ? '复核不通过将归档当前工作流。请填写依据。' : '请填写复核或修订依据，操作会保留历史。'}</p><Input.TextArea aria-label="操作依据" rows={3} value={edit?.note} onChange={(e) => setEdit({ ...edit, note: e.target.value })} />
    </Modal>
