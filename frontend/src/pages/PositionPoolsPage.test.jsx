...
const membership = { id: 7, revision: 4, status: 'pending_review', tags: [], assessment: { standard: { name: '机械工程师投递' }, pool: { name: '机械工程师池' }, tag_catalog: [] }, events: [] }
...
describe('PositionPoolsPage', () => {
  ...
  it('requires review evidence and submits the membership revision for approval', async () => {
    render(<PositionPoolsPage />)
    await userEvent.click(screen.getByRole('button', { name: '查看详情' }))
    await userEvent.click(await screen.findByRole('button', { name: '复核通过并分配' }))
    ...
    await waitFor(() => expect(actOnPoolMember).toHaveBeenCalledWith(7, 'review', { revision: 4, decision: 'approve', note: '原文证明符合机械设计要求' }))
  })
