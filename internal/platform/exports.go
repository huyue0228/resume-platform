...
func (a *App) importTemplate(w http.ResponseWriter, r *http.Request, key string) error {
	...
	if key == "contacts" {
		instructions = append(instructions,
			...
			[]any{"重复授权", "同一工号、部门和角色重复出现且设置相同时自动合并；可转派或是否启用设置冲突时，提示冲突行号并取消整批导入。不同部门或角色的授权分别保留。"},
...
func (a *App) resultReport(w http.ResponseWriter, r *http.Request, p *Principal) error {
	...
	statuses := []string{"raw", "talent_pool", "archived", "pending_reallocation", "pending_allocation", "pending_dispatch", "pending_screening", "screening_passed", "screening_rejected"}
