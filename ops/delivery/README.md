...
本版本面向新环境，使用独立数据库和媒体目录，配置 HTTPS 域名及 W3 OAuth2；模型连接在授权的系统设置页面填写。平台与 Kernel 必须同时支持协议包 5.0.0（resume-analysis/v5、resume-allocation/v1）。所有范围使用独立分配；不提供旧词表、HC 容量或历史分配模式迁移。不要覆盖已有环境或删除已有数据卷。
