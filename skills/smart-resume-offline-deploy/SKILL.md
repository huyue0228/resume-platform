...
本版只面向新数据库，不支持 v4 或更旧数据库原位升级。旧实例保持不动，选择独立的 `COMPOSE_PROJECT_NAME` 和空数据卷部署 v5，同机使用空闲端口；不得删除旧卷或覆盖旧配置。下文的已有部署升级菜单和项目名沿用规则，仅适用于维护已经建立的 v5 实例。先核对随包 `README-offline-deploy.md` 的版本与范围。
