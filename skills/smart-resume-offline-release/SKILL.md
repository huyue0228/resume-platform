...
先设置 `AGENT_KERNEL_IMAGE` 和 `AGENT_KERNEL_VERSION`，并拉取或加载已独立发布的 Kernel 镜像。脚本自动完成：生成平台时间戳版本、构建五个 `linux/amd64` 平台镜像（含备份工具，不构建 Kernel）、将指定 Kernel 镜像一起封装、容器内检查、生成纯 `image:` Compose 离线包、计算双层 SHA-256、回读镜像、复制到移动硬盘并复验。
...
- backend、frontend、PostgreSQL、Redis、backup 与独立 Kernel 镜像均为 `linux/amd64`。
