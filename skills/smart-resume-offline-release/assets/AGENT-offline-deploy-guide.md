...
模型路由器的 CA 是独立验收项：平台模型 TEST 当前设置 `verify=False`，通过 TEST 或 `/healthz` 不能证明 Kernel 的 TLS 信任链正常。模型路由器使用企业 CA 时，在 `.env` 设置 `AGENT_KERNEL_CA_BUNDLE` 为现场 CA PEM 文件绝对路径，由部署 Skill 自动只读挂载到 Kernel 并设置 `SSL_CERT_FILE`；保持 TLS 校验开启。检查 `agent` 用户可读权限，更新 CA 后重建 Kernel。最终必须完成一次真实 Agent 分析；仅模型 TEST 成功不得报告 Agent 功能验收通过。详见随包部署 Skill 的企业 CA 小节。
