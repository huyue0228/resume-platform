...
首次运行选择“创建 `.env`、自动生成密钥并退出”。脚本会固定使用包内已确定的镜像、端口、并发、OCR、数据库标识，并自动生成 `DJANGO_SECRET_KEY`、`POSTGRES_PASSWORD`、`USAGE_METRICS_TOKEN` 和 `AGENT_KERNEL_TOKEN`。部署人员只需：

1. 把生产域名写入 `DJANGO_ALLOWED_HOSTS`，配置 DNS、可信 TLS 证书和 HTTPS 反向代理或企业网关；所有路径统一转发到 app 暴露端口，不直接暴露 backend `8000`。同机反代建议将 `FRONTEND_BIND` 改为 `127.0.0.1`。
...
启动后应有 `app`、`agent-kernel`、`db`、`redis` 四个常驻容器。app 内部独立运行 Nginx、API、default/prefork 与 ai/threads 进程；`application.py --healthcheck` 检查 Web、API 和两个队列。查看业务日志使用 `docker compose logs -f app`。升级时沿用原 `COMPOSE_PROJECT_NAME`，更新 `APP_VERSION` 和 app 镜像引用；不再使用 `BACKEND_IMAGE`/`FRONTEND_IMAGE`，保留原密钥、W3 和 Kernel/CA 配置。

**模型路由器使用企业 CA 时，必须额外配置 Kernel 的 CA。** 将现场已有的 CA PEM 文件绝对路径写入 `.env` 的 `AGENT_KERNEL_CA_BUNDLE`，保持 `AGENT_KERNEL_MODEL_INSECURE_SKIP_VERIFY=False`。部署脚本自动只读挂载并设置 `SSL_CERT_FILE`；文件须可被容器内 `agent` 用户读取。CA 内容更新后需重建 Kernel 容器。

平台模型 TEST 当前跳过 TLS 校验，TEST 成功不代表 Kernel 信任了模型路由器。`verify.sh` 只检查 CA 挂载是否可读，还必须从简历库完成一次真实 Agent 分析。完整命令及直接使用 Compose 时的覆盖文件要求见 [部署 Skill](smart-resume-offline-deploy-skill/SKILL.md)。
